...
import { canAcceptExternalSse, trackExternalSse } from "./memoryCache";
import { db } from "./db";
import { 
  adAccounts, 
  campaigns, 
...
<body>
  ...
  <div class="footer">
    ...
    // Use Puppeteer to generate PDF with system Chromium
...
const MAX_ADS_SSE_CONNECTIONS = 20;
...
router.get("/analytics/live", requireAdvertiser, async (req, res) => {
  try {
    if (activeLiveConnections.size >= MAX_ADS_SSE_CONNECTIONS || !canAcceptExternalSse()) {
    ...
    req.on('close', cleanupAds);
    req.on('error', cleanupAds);
    
  } catch (error) {
    if (activeLiveConnections.has(res)) {
      activeLiveConnections.delete(res);
      trackExternalSse(-1);
    }
