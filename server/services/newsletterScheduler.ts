...
class NewsletterScheduler {
  ...
  public start() {
    ...
    if (this.isRunning || globalThis._newsletterSchedulerStarted) {
      console.log('[NewsletterScheduler] Scheduler already running (global guard active)');
      return;
    }
  ...
  private async executeScheduledNewsletter(config: ScheduleConfig) {
    ...
    if (globalThis._newsletterExecutingJobs.has(lockKey)) {
      console.log(`[NewsletterScheduler] ⚠️ Job ${config.type} is already executing, skipping concurrent run`);
      return;
    }
    ...
    const releaseLock = async () => {
      ...
      try {
        ...
        console.log(`[NewsletterScheduler] 🔓 Released DB lock for ${config.type}`);
        hasDbLock = false;
      } catch (e) {
    ...
    try {
      ...
      if (!hasDbLock) {
        console.log(`[NewsletterScheduler] ⚠️ Could not acquire DB lock for ${config.type}, another pod is processing`);
        globalThis._newsletterExecutingJobs.delete(lockKey);
        return;
      }
      console.log(`[NewsletterScheduler] 🔒 Acquired DB lock for ${config.type} (hash: ${lockHash})`);
      ...
      if (existingToday.length > 0) {
        console.log(`[NewsletterScheduler] ⚠️ Newsletter ${config.type} already sent today (${existingToday[0].id}), skipping duplicate`);
        await releaseLock();
        globalThis._newsletterExecutingJobs.delete(lockKey);
        return;
      }
      
      console.log(`[NewsletterScheduler] ✅ No duplicate found for ${config.type} today, proceeding...`);
  ...
  private async sendToSubscribers(
    ...
    try {
      ...
      console.log(`[NewsletterScheduler] Generated daily question: ${dailyQuestion.question}`);
      ...
      console.log(`[NewsletterScheduler] Generated AI subject line: ${selectedSubject?.subject || 'none'}`);
      ...
      console.log(`[NewsletterScheduler] Cached ${summaryCache.size} enhanced article summaries`);
