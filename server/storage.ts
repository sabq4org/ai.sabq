...
export class DatabaseStorage implements IStorage {
  ...
  async getCategoriesWithStats(): Promise<Array<Category & {
    ...
    return rows.map((r: any) => ({
      ...
      seasonalRules: r.seasonal_rules,
      features: r.features,
  ...
  async getHeroArticles(): Promise<ArticleWithDetails[]> {
      ...
      .where(
        and(
          ...
          or(
            isNull(articles.articleType),
            ne(articles.articleType, 'opinion'),
            eq(articles.isFeatured, true)  // Allow starred opinion articles in hero
          ),
          or(
    ...
    return results.map((r) => ({
      ...
      infographicBannerUrl: r.infographicBannerUrl || null,
  ...
  async getAllPublishedArticles(limit: number = 20, offset: number = 0): Promise<ArticleWithDetails[]> {
    ...
    const results = await db
      ...
      .limit(limit * 3) // Fetch more to account for duplicates from LEFT JOIN
      .offset(offset);

    // Deduplicate results (LEFT JOIN with storyLinks can cause duplicates)
    const seenIds = new Set<number>();
    const uniqueResults = results.filter((r) => {
      if (seenIds.has(r.id)) return false;
      seenIds.add(r.id);
      return true;
    }).slice(0, limit); // Limit to requested count after deduplication
