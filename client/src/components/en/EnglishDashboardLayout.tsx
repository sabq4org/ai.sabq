...
export function EnglishDashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const renderNavItem = (item: NavItem) => {
    ...
    // Helper to get display label - prefer labelEn for English dashboard
    const getLabel = (navItem: NavItem) => navItem.labelEn || navItem.labelKey;
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground truncate">
                    {user?.role === "admin" ? "Admin" : user?.role === "editor" ? "Editor" : "Writer"}
                  </p>
                </div>
              </div>
              <Button
